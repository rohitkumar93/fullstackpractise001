...
def ingest_sample_documents():
    ...
    sample_docs = [
        ...
        # Non-AI documents for ranking validation
        {
            ...
            "content": "The football team won the championship after a tough season.",
        },
        {
            "filename": "cooking_guide.pdf",
            "content": "Learn how to cook delicious meals with simple ingredients.",
        },
    ]
    ...
    for doc in sample_docs:
        # Store document
        document = Document(
            filename=doc["filename"], content=doc["content"].encode("utf-8")
        )
        db.add(document)
        db.commit()
        ...
        assert embedding_vector.shape[0] == 384, (
            f"❌ Vector size mismatch! Got {embedding_vector.shape[0]} instead of 384."
        )

        # Store embedding
        embedding_entry = Embedding(
            document_id=document.id, vector=embedding_vector.tolist()
        )
