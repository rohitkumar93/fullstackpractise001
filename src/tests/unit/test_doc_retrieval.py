...
async def test_get_document_texts():
    ...
    mock_db.__aenter__.return_value = (
        mock_db  # __aenter__ should return the mock_db itself
    )
    ...
    mock_scalars_result.all.return_value = [
        ...
        "doc2",
        "doc3",
    ]  # Mock the return value of all()
    mock_execute_result.scalars.return_value = (
        mock_scalars_result  # Mock the return value of scalars()
    )
    mock_db.execute.return_value = (
        mock_execute_result  # Mock the return value of execute()
    )
...
async def test_get_document_texts_no_matching_docs():
    ...
    with patch(
        ...
        # Call the method with the correct arguments (document_ids)
        document_texts = await retrieval_service.get_document_texts(
            [99, 100]
        )  # Non-existent IDs
