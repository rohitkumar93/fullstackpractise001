...
async def test_process_document_valid_string():
    ...
    with patch.object(
        service.embedding_generator, "generate_embedding", new_callable=AsyncMock
    ) as mock_embedding:
...
async def test_process_document_valid_bytes():
    ...
    with patch.object(
        ...
        response = await service.process_document(
            "test.txt", b"This is a test document."
        )
        assert response == {"message": "Document processed successfully"}
...
async def test_process_document_invalid_embedding():
    ...
    with patch.object(
        ...
        with pytest.raises(
            TypeError, match="Embedding vector must be a list of floats/ints."
        ):
...
async def test_process_documents_batch():
    ...
    documents = [
        DocumentUploadRequest(
            filename=f"test_doc_{i}.txt", content=f"This is test document number {i}."
        )
        for i in range(3)
    ]
