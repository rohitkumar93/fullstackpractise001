...
async def test_get_answer_valid_query():
    ...
    with patch.object(service.retrieval_service, 'retrieve_relevant_docs', new_callable=AsyncMock) as mock_retrieve_docs, \
         patch.object(service.retrieval_service, 'get_document_texts', new_callable=AsyncMock) as mock_get_texts, \
         patch('openai.ChatCompletion.acreate', new_callable=AsyncMock) as mock_openai:
        
        mock_retrieve_docs.return_value = [1, 2, 3]
        mock_get_texts.return_value = ["Document 1 text", "Document 2 text", "Document 3 text"]
        mock_openai.return_value = {"choices": [{"message": {"content": "AI is the simulation of human intelligence in machines."}}]}
        
        response = await service.get_answer(request)
        assert response.answer == "AI is the simulation of human intelligence in machines."
...
async def test_get_answer_no_relevant_docs():
    ...
    with patch.object(service.retrieval_service, 'retrieve_relevant_docs', new_callable=AsyncMock) as mock_retrieve_docs, \
        ...
        mock_openai.return_value = {"choices": [{"message": {"content": "No relevant documents found."}}]}
